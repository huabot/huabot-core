...
def encode_url(url, encoding="UTF-8"):
    RE_ZH_CN = re.compile('[^a-z0-9%\?#&/:\.=]', re.I)
    words = RE_ZH_CN.findall(url)
    for w in set(words):
        url = url.replace(w, quote(bytes(w, encoding)))

    return url


def daemonize(home_dir='.', umask=22):
    try:
        pid = os.fork()
        if pid > 0:
            sys.exit()
...
def redirect(stdin, stdout, stderr=None):
